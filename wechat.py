...
class WebWeixin(object):
	...
	def __init__(self):
		...
		self.credibleUser = ['石天鑫','马良','妈妈','石寿玉','杨君然','杨科','顾袁超','兰宇航','刘宇航','陈宏斌','郭李','李腾飞','赵承运']
		...
		for msg in r['AddMsgList']:
			...
			if msgType == 51:
				...
				if msg['ToUserName'] == 'filehelper':
					...
					print name+'(发送了): '+content
					if name in self.credibleUser:
						ans = self._tuning(content) + '[AI]'
	...
	def _tuning(self, word):
		url = 'http://www.tuling123.com/openapi/api?key=%s&info=%s' % ('2ba083ae9f0016664dfb7ed80ba4ffa0', word)
		r = requests.get(url)
		ans = r.json()
		if ans['code'] == 100000: return ans['text']
		else: return "现在思路很乱，最好联系下我哥 T_T..."
